...
figtypes = ['bsn_deriv_acc', 'slm_with_derivs_acc']

pdf = matplotlib.backends.backend_pdf.PdfPages(os.path.join(userTracker.codePath(), "comparison_details.pdf"))
...
for key in keys:
    ...
    for row, figtype in enumerate(figtypes):
        ...
        ts.set_title(figtype+r' $R^2_\mathrm{test}(\mathrm{velocity})$ = %0.3f, $R^2_\mathrm{test}(\mathrm{acceleration})$ = %0.3f' % (res['scorepredicted'], deriv_r2(data[key][figtype]['signal'], data[key][figtype]['output'], data[key][figtype]['test_idx'])))
        ...
        sc.plot(res['signal'][res['train_idx']], res['output'][res['train_idx']], 'go', label = 'Train')
        sc.plot(res['signal'][res['test_idx']], res['output'][res['test_idx']], 'bo', label = 'Test')
        sc.plot([min(res['signal']), max(res['signal'])], [min(res['signal']), max(res['signal'])], 'k-.')
        sc.set_title(figtype+r' $\rho_\mathrm{test}(\mathrm{velocity})$ = %0.3f' % (res['corrpredicted']))
    ...
    slm_weights = data[key]['slm_with_derivs_acc']['weights']*np.sqrt(data[key]['slm_with_derivs_acc']['variance'])
    bsn_weights = data[key]['bsn_deriv_acc']['weights']*np.sqrt(data[key]['slm_with_derivs_acc']['variance'])

    n = slm_weights.size/3
    ...
    ax.text(-2, 1.8, r'$\left.\frac{dF}{dt}\right|_+$', fontsize=16)
