...
def optimize_slm(time, Xfull, Yfull, options = None):
    ...
    if not options['decision_tree']:
        if not options['best_neuron']:
            ...
            if not options['derivative_penalty']:
                ...
                return {'weights'   : weights,
                        ...
                        'signal'    : Yfull,
                        'prediction': f(Xfull[best_neuron_idx,:], best_neuron_params),
                        'time'      : time,
                        'train_idx' : train_idx,
                        'test_idx'  : test_idx
                        }
        ...
        prediction_full[decision_full > 0] = np.dot(Xfull[:,decision_full>0].T, res1['weights']) + res1['offset']
        prediction_full[decision_full < 0] = np.dot(Xfull[:,decision_full<0].T, res2['weights']) + res2['offset']
...
if __name__ == '__main__':
    ...
    for typ_cond in ['AKS297.51_moving']:#, 'AML32_moving']:#, 'AML70_chip', 'AML70_moving', 'AML18_moving']:
        ...
        for key in keyList:
            ...
            r2s_train = [r2s[k]['R2_train'] for k in r2keys]
            r2s_test  = [r2s[k]['R2_test'] for k in r2keys]
