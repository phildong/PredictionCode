...
def plot_candidate(x,  x_name, metric_name = r'$R^2_{\mathrm{ms},\mathrm{test}}$', labels=None, PDF=None, ylim=[0,1]):
    fig, ax = plt.subplots(figsize=(7,7))
    ax.plot(x, r2ms, 'o', markersize=16)

    #adapted from: https://stackoverflow.com/questions/18767523/fitting-data-with-numpy
    try:
        coefs = np.polyfit(x, r2ms, 1)
    ...
    ax.set_title('Corrcoef =%.2f' %np.corrcoef(x, r2ms)[0,1] )
    ax.tick_params(axis='both', which='major', labelsize=19)
    for i, txt in enumerate(labels):
        ax.annotate(txt, (x[i], r2ms[i]))
...
for behavior in ['velocity', 'curvature']:
    filename = outputFolder + '/correlates_of_performance_' + behavior + '.pdf'
    ...
    for typ_cond in ['AML310_moving', 'AML32_moving']:
        ...
        for key in keyList:
            ...
            r2ms.append(res['R2ms_test'])
            bsn_r2ms.append(bsnres['R2ms_test'])
            ...
            mse = compare_pdf(beh_test, beh, alabel="test", blabel="train", suplabel=key + "R^2_ms = %.2f " % res['R2ms_test'], PDF=pdf)
    ...
    plot_candidate(bsn_r2ms, " Best Single Neuron r2ms",
