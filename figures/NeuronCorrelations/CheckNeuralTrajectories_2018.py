...
for typ_cond in ['AML32_chip']: #, 'AKS297.51_moving']:
    ...
    frac_nan_during_imm = np.true_divide(np.sum(np.isnan(neurons_withNaN[:,im_start:]),axis=1), neurons_withNaN[:,im_start:].shape[1])
    valid_imm = np.argwhere(frac_nan_during_imm < 0.25)[:,0]
    Neuro = np.copy(neurons[valid_imm, imm_start_index: end_index]).T # I_smooth_interp_nonctoig
    ...
    plt.figure()
    plt.subplot(3, 1, 1)
    plt.plot(dset['Neurons']['TimeFull'], beh, 'k', label='velocity')
    ...
    plt.ylabel('Velocity')
    plt.subplot(3, 1, 2)
    plt.plot(time[imm_start_index:end_index], pcs_z[:, 0], label='PC0 z')
    plt.plot(time[imm_start_index:end_index], 3 + pcs_z[:, 1], label='PC1 z')
    plt.plot(time[imm_start_index:end_index], 6 + pcs_z[:, 2], label='PC2 z')
    plt.xlim(time[imm_start_index], time[end_index])
    plt.legend()
    plt.subplot(3, 1, 3)
    plt.plot(time[imm_start_index:end_index], pcs_dFdt_z[:, 0], label='PC0 z dF/dT')
    plt.plot(time[imm_start_index:end_index], pcs_dFdt_z[:, 1], label='PC1 z dF/dT')
    plt.plot(time[imm_start_index:end_index], pcs_dFdt_z[:, 2], label='PC2 z dF/dT')
    plt.xlim(time[imm_start_index], time[end_index])
    plt.legend()
