...
for typ_cond in ['AML310_transition']: #, 'AML310_moving']:
    ...
    data, immobile_withNaN, immobile_zscore, neuron_number, velocity_moving = get_data(0,950, neurons, neurons_withNaN, neurons_ZScore, time, time_contig, velocity) #1305-2205 is the green immobile part of the data
    ...
    #plt.colorbar()
    plt.title('Calcium Signal_193044')
    plt.xlabel('Time (sec/6)')
    plt.ylabel('Sorted Neuron')

    plot11 = plt.figure(4)
    plt.plot(velocity_moving) #, moving_time)
    plt.title('Velocity')
    plt.xlabel('frames')
