...
for rank in np.concatenate([np.arange(10), np.arange(num_neurons-3,num_neurons)], axis=0):
    ...
    fig1.suptitle(key + ' Neuron: %d, Weight Rank: %d, Weight = %.2f' % (neuron, rank, weight))
    gs = gridspec.GridSpec(ncols=4, nrows=2, figure=fig1)
    f1_ax1 = fig1.add_subplot(gs[0, 0], xlabel=behavior, ylabel='Activity')
    f1_ax1.plot(vel_data[key], activity, 'o')
